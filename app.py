...
# Solo incluye lo que el cliente necesita decidir y pagar.
# Prioridad: multas > retenciones vencidas > IGV/Renta vencidos > fraccionamientos
# NO incluye trabajo interno (actualizar reporte, pagados con atraso registrados).
# ══════════════════════════════════════════════════════════════════════════════
def generar_informe_cliente(empresa, res, hoy):
    vm=res["vm"]; vn=res["vn"]
    lines=[]
    lines.append(f"📋 *ESTADO DE OBLIGACIONES TRIBUTARIAS*")
    lines.append(f"*{empresa.upper()}*")
    lines.append(f"_{hoy.strftime('%d de %B de %Y').title()}_")
    lines.append("")

    total_urgente = sum(r["importe"]+r["tim"]+r["multa"] for r in vm+vn
                       if not r.get("es_manual"))

    if not vm and not vn:
        lines.append("✅ *Todo al día. No hay pagos pendientes urgentes.*")
        lines.append("")
        lines.append("_Contadeus International SAC_")
        return "\n".join(lines)

    # 1. RETENCIONES CON MULTA (máxima urgencia)
    if vm:
        lines.append("🚨 *URGENTE — Retenciones vencidas (riesgo de multa adicional):*")
        lines.append("_Pagar cuanto antes para evitar multa del 100% del tributo omitido._")
        for r in vm:
            costo=r["importe"]+r["tim"]+r["multa"]
            vs=r["fecha_venc"].strftime("%d/%m/%Y") if r.get("fecha_venc") else ""
            lines.append(
                f"• *{r['nombre']} {r['periodo']}*: S/ {r['importe']:,.2f}"
                f" + Multa S/ {r['multa']:,.0f} + TIM S/ {r['tim']:,.2f}"
                f" = *Total S/ {costo:,.2f}*"
                f" _(venció {vs})_"
            )
        lines.append("")
    ...
    vn_normal=[r for r in vn if not r.get("es_manual") and r["codigo"]!="8021"]
    vn_fracc =[r for r in vn if r["codigo"]=="8021"]
    vn_manual=[r for r in vn if r.get("es_manual")]
    ...
    if vn_normal:
        ...
        for r in vn_normal:
            ...
            lines.append(
                f"• *{r['nombre']} {r['periodo']}*: S/ {r['importe']:,.2f}"
                f" + TIM S/ {r['tim']:,.2f} = *S/ {costo:,.2f}*"
                f" _(venció {vs}, {r['dias_tarde']} días)_"
            )
        lines.append("")

    if vn_fracc:
        lines.append("🔴 *Cuotas de fraccionamiento vencidas:*")
        for r in vn_fracc:
            vs=r["fecha_venc"].strftime("%d/%m/%Y") if r.get("fecha_venc") else ""
            lines.append(
                f"• *{r['periodo']}*: S/ {r['importe']:,.2f}"
                f" + TIM S/ {r['tim']:,.2f}"
                f" _(venció {vs})_"
            )
        lines.append("")
    ...
    if vn_manual:
        lines.append("📋 *Pagos manuales pendientes (AFP/SIS):*")
        for r in vn_manual:
    ...
    if total_urgente>0:
        lines.append("─"*35)
        lines.append(f"💰 *TOTAL A REGULARIZAR: S/ {total_urgente:,.2f}*")
        lines.append("")
        lines.append("📌 *Recomendación de prioridad de pago:*")
        if vm:
            lines.append("  1️⃣ Retenciones con multa — *máxima urgencia*")
        if vn_normal or vn_fracc:
            n=2 if vm else 1
            lines.append(f"  {n}️⃣ Impuestos vencidos — interés crece cada día")
        if vn_manual:
            n=(3 if vm and (vn_normal or vn_fracc) else 2 if vm or vn_normal or vn_fracc else 1)
            lines.append(f"  {n}️⃣ AFP/SIS — coordinar pago")
        lines.append("")
        lines.append("⚠️ _Los intereses (TIM) aumentan 0.04% diario._")
        lines.append("_Contáctenos para orientarle en el proceso de pago._")
...
# ── FORMULARIO ──────────────────────────────────────────────────────────────
    ...
    with col_h:
        ...
        st.markdown("""<div class="card cr" style="margin-top:10px;">
            <strong>⚠️ Retenciones con multa 5% UIT</strong><br>
            ONP · Renta 5ta · Renta 4ta · Renta 2da<br>
            <em>UIT 2022=S/4,600 · 2023=S/4,950 · 2024=S/5,150</em><br>
            <em>UIT 2025=S/5,350 · 2026=S/5,500</em>
        </div>""",unsafe_allow_html=True)
