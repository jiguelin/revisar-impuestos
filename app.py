...
# Fuente oficial SUNAT: tabla de tasas. Desde 01/04/2021 la TIM mensual MN es 0.9%.
...
# Tributos retenidos/aportes con alerta de multa por pago fuera de plazo.
# No se calcula monto de multa: Contadeus revisa infracción/gradualidad manualmente.
...
def add_months(d, months):
    """Suma meses calendario conservando día; si el mes no tiene ese día, usa el último."""
...
def fvenc_ij(anio, mes, dig):
    """
    IGV Justo: prórroga del pago original por 3 meses.
    Si el reporte trae la fecha máxima del padrón SUNAT, esa fecha manda.
    Esta función solo es fallback conservador cuando no se digitó/leyó fecha máxima.
    """
    return add_months(fvenc(anio, mes, dig), 3)

def tim_diaria(fecha_ref):
    if not isinstance(fecha_ref, date):
        fecha_ref = date.today()
    for desde, tasa_dia in TIM_HIST:
        if fecha_ref >= desde:
            return tasa_dia
    return 0.0120/30

def ctim(imp, dias, fecha_ref=None):
    return round(imp * tim_diaria(fecha_ref or date.today()) * max(dias, 0), 2)
...
def parse_igv_justo_manual(txt):
    """
    Lee fechas máximas pegadas manualmente. Formatos aceptados:
      202603=24/07/2026
      MARZO 2026 24/07/2026
      03/2026 24/07/2026
    Devuelve {(anio, mes): fecha_maxima}.
    """
    out={}
    for raw in str(txt or "").splitlines():
        ...
        f=parse_fecha(line)
        if not f: continue
        ym=re.search(r"(20\d{2})[-/]?(0?[1-9]|1[0-2])", line)
        if ym:
            out[(int(ym.group(1)), int(ym.group(2)))] = f
            continue
        my=re.search(r"(0?[1-9]|1[0-2])/(20\d{2})", line)
        if my:
            out[(int(my.group(2)), int(my.group(1)))] = f
            continue
        for nom, mv in MESES.items():
            if nom in line:
                ay=re.search(r"20\d{2}", line)
                if ay:
                    out[(int(ay.group(0)), mv)] = f
                    break
    return out
...
def parsear_reporte(df):
    ...
    for _,row in df.iterrows():
        ...
        igv_j=None
        if st_=="A":
            # Si el reporte incluye la fecha máxima del padrón IGV Justo, úsala.
            # Busca cualquier fecha en la fila y toma la mayor, porque normalmente
            # la fecha prorrogada es posterior al vencimiento original.
            fechas=[parse_fecha(c) for c in v if parse_fecha(c)]
            if fechas:
                igv_j=max(fechas)
...
# ══════════════════════════════════════════════════════════════════════════════
def analizar(extracto, reporte, digito, igv_justo, hoy, igv_justo_manual=None):
    igv_justo_manual = igv_justo_manual or {}
    ...
    for _,fila in extracto.iterrows():
        ...
        if cod=="1011" and igv_justo:
            fij=igv_justo_manual.get((anio, mes)) or rec_rep.get("igv_justo")
        ...
        mul_v=0.0
        alerta_multa=afecta_multa(cod, dt)
        ...
        base={"codigo":cod,"nombre":TRIBUTO[cod],
              ...
              "dias_tarde":dt,"tarde":tarde,"tim":tim_v,"multa":0.0,
              "alerta_multa":alerta_multa,"tiene_multa":alerta_multa,"n_orden":nord,"ya_reg":ya,
    ...
    for r in reporte:
        ...
        if cod=="8021":
            ...
            vn.append({
                ...
                "dias_tarde":dv,"tarde":True,"tim":ctim(r["declarado"],dv,fv),"multa":0.0,
        ...
        if cod=="1011" and igv_justo:
            fij=igv_justo_manual.get((anio, mv)) or r.get("igv_justo")
            fv=fij or fvenc_ij(anio,mv,digito); tv="IGV Justo"
        else:
        ...
        mul_v=0.0
        alerta_multa=afecta_multa(cod, dv)
        item={
            "codigo":cod,"nombre":TRIBUTO.get(cod,""),
            "periodo":f"{mn}-{anio}","mes":mn,"mes_num":mv,"anio":anio,
            "importe":saldo,"fecha_pago":None,"fecha_venc":fv,"tipo_venc":tv,
            "dias_tarde":dv,"tarde":True,"tim":tim_v,"multa":0.0,
            "alerta_multa":alerta_multa,"tiene_multa":alerta_multa,"n_orden":"","ya_reg":True,
...
NOTA_CONCEPTO = {
    "3052": "retención de remuneración — multa adicional si detecta SUNAT",
    "3042": "retención de honorarios — multa adicional si detecta SUNAT",
    "3022": "retención — multa adicional si detecta SUNAT",
    "5310": "aportación previsional del trabajador — no fraccionable",
    "8021": "compromiso con SUNAT — incumplimiento reactiva deuda original",
    "AFP":  "obligación previsional del trabajador — no fraccionable",
    "SIS":  "cronograma especial — seguimiento independiente",
}
...
def generar_informe_cliente(empresa, res, hoy):
    ...
    lines=[]
    lines.append(f"📊 *Reporte Tributario — {empresa}*")
    lines.append(f"Contadeus International SAC · {hoy.strftime('%d/%m/%Y')}")
    lines.append("")
    ...
    for i, r in enumerate(ordenados, 1):
        monto = round(r["importe"] + r["tim"])
        total += monto
        concepto = CONCEPTO_CLIENTE.get(r["codigo"], r["nombre"])
        periodo  = r["periodo"]

        # Detalle según tipo
        if r.get("alerta_multa"):
            detalle = "_(afecto a multa — revisar gradualidad)_"
        elif r.get("es_manual") or r["dias_tarde"] == 0:
            nota = NOTA_CONCEPTO.get(r["codigo"], "")
            detalle = f"_({nota})_" if nota else ""
        elif r["dias_tarde"] > 0:
            detalle = f"_({r['dias_tarde']} días de retraso, interés incluido)_"
        else:
            detalle = ""
...
def gen_excel(empresa,ruc,res,extracto,noms,fuente):
    ...
    t_urgente=sum(r["importe"]+r["tim"] for r in vm+vn)
    ...
    with pd.ExcelWriter(buf,engine="openpyxl") as w:
        ...
        pd.DataFrame({"Campo":[
            ...
            "Años analizados","Vencidos con alerta de multa","Vencidos sin pagar",
        ...
        def rows(lst,extras):
            ...
            for r in lst:
                ...
                for k in extras:
                    ...
                    elif k=="Alerta multa": d[k]="Afecto a multa - revisar gradualidad" if r.get("alerta_multa") else "No aplica"
        ...
        if vm:  pd.DataFrame(rows(vm, ["Vencimiento","Días","TIM S/","Alerta multa","Total S/"])).to_excel(w,sheet_name="🔴 VENCIDOS+MULTA",index=False)
...
<div class="hdr">
  ...
  <p>IGV Justo corregido · TIM SUNAT vigente · Multas como alerta, sin monto exacto</p>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.listo and st.session_state.res:
    ...
    for col,(n,lbl,color) in zip(st.columns(5),[
        (len(vm),"Vencidos + alerta","red"),(len(vn),"Vencidos sin pagar","red"),
    ...
    if vm:
        st.markdown('<div class="stitle">🚨 URGENTE — Retenciones vencidas con alerta de multa</div>',
                    unsafe_allow_html=True)
    ...
    if act:
        ...
        for anio in anios:
            ...
            for r in items:
                atr_txt=""
                if r["tarde"]:
                    ml=f'<span class="badge br">Afecto a multa</span>' if r.get("alerta_multa") else ""
    ...
    if atr:
        st.markdown('<div class="sep"></div>',unsafe_allow_html=True)
        with st.expander(f"🟠 {len(atr)} pagados con atraso — TIM y alerta de multa"):
            for anio in anios:
                items=[r for r in atr if r["anio"]==anio]
                if not items: continue
                st.markdown(f'<div class="yr">📅 {anio}</div>',unsafe_allow_html=True)
                for r in items:
                    ml="⚠️ Afecto a multa - revisar gradualidad" if r.get("alerta_multa") else "Solo TIM"
...
# ── FORMULARIO ──────────────────────────────────────────────────────────────
    ...
    with col_f:
        ...
        igv_chk=st.checkbox("✅ Aplicar IGV Justo cuando el reporte tenga fecha máxima o corresponda usar fallback 3 meses",value=True)
        st.caption("Recomendado: pega/coloca en tu reporte la fecha máxima del padrón IGV Justo. Si no existe fecha, el sistema usa como fallback el vencimiento original + 3 meses.")
        igv_manual_txt=st.text_area(
            "Fechas máximas IGV Justo del padrón SUNAT (opcional)",
        ...
        if st.button("🔍  Analizar",type="primary",use_container_width=True):
            ...
            if not err and digito_calc is not None:
                ...
                with st.spinner("Analizando 2022–2026..."):
                    ...
                    igv_manual=parse_igv_justo_manual(igv_manual_txt)
                    resultado=analizar(edf,rp,digito_calc,igv_chk,HOY,igv_manual)
    ...
    with col_h:
        st.markdown("""<div class="card cb" style="margin-top:8px;">
            ...
            🚨 Retenciones vencidas (alerta de multa)<br>
        ...
        st.markdown("""<div class="card cr" style="margin-top:10px;">
            <strong>⚠️ Retenciones con alerta de multa</strong><br>
            ONP · Renta 5ta · Renta 4ta · Renta 2da<br>
            <em>No calcula multa exacta. Contadeus revisa infracción y gradualidad.</em>
        </div>""",unsafe_allow_html=True)

        st.markdown("""<div class="card cg" style="margin-top:10px;">
            <strong>📅 Cronogramas verificados</strong><br>
            2022 · 2023 · 2024 · 2025 · 2026<br>
            IGV Justo: fecha del padrón o fallback +3 meses<br>
