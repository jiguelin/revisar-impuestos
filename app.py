...
# TIM SUNAT - Moneda Nacional.
# La tasa mensual vigente desde 01/04/2021 es 0.9%; tasa diaria = mensual / 30.
# Se mantiene histórico básico para pagos atrasados de años anteriores.
TIM_HIST = [
    (date(2021,4,1), 0.0090/30),
    (date(2020,4,1), 0.0100/30),
    (date(2010,3,1), 0.0120/30),
]

# Tributos retenidos/aportes que, si se pagan fuera de plazo, quedan AFECTOS A MULTA.
# No se calcula monto ni porcentaje de multa: Contadeus revisa gradualidad manualmente.
CON_MULTA = {"3052","3042","3022","5310"}

TRIBUTO = {
    "1011":"IGV","3031":"Impuesto a la Renta","3111":"Impuesto a la Renta",
    "3121":"Impuesto a la Renta","3038":"ITAN","3052":"Renta 5ta Categoría",
    "3042":"Renta 4ta Categoría","3022":"Renta 2da Categoría",
    "5210":"EsSalud","5310":"ONP","8021":"Fraccionamiento Art.36",
    "AFP":"AFP","SIS":"SIS",
    "PLAME":"Planilla mensual (EsSalud/AFP)",  # Formulario 1663
}
...
def add_months(d, months):
    """Suma meses calendario. Si el mes destino no tiene ese día, usa el último día."""
...
def fvenc_ij(anio, mes, dig):
    """Fallback IGV Justo: vencimiento original del periodo + 3 meses."""
    return add_months(fvenc(anio, mes, dig), 3)

def tim_diaria(fecha_ref=None):
    fecha_ref = fecha_ref if isinstance(fecha_ref, date) else date.today()
    for desde, tasa in TIM_HIST:
        if fecha_ref >= desde:
            return tasa
    return 0.0120/30

def ctim(imp, dias, fecha_ref=None):
    return round(imp * tim_diaria(fecha_ref) * max(dias,0), 2)

def afecta_multa(cod, dias):
    return cod in CON_MULTA and dias > 0
...
SMAP=[
    ...
    # "PAGO DE RETENEDOR" = retención de renta (tipo 4ta/5ta según empresa)
    # Se mapea a 3052 (5ta) por ser el más común, pero sin código exacto
    (r"PAGO.*RETENED|RETENED.*PAGO|RETENCI[OÓ]N.*RENTA",        "3052","B"),
    (r"\bAFP\b",                                                  "AFP","C"),
    (r"\bSIS\b",                                                  "SIS","C"),
]
...
def parsear_reporte(df):
    ...
    for _,row in df.iterrows():
        ...
        igv_j=None
        if st_=="A":
            # El reporte de impuestos es la primera fuente para IGV Justo.
            # Si la fila trae una fecha máxima de IGV Justo, se usa esa fecha.
            # Tomamos la fecha más alta de la fila, porque normalmente es posterior
            # al vencimiento original y a otras fechas auxiliares.
            fechas=[parse_fecha(c) for c in v]
            ...
            for pg in pdf.pages:
                for tbl in pg.extract_tables():
                    ...
                    for row in tbl[1:]:
                        ...
                        # Formulario 1663 (PLAME): tributo="-", asignar código PLAME
                        form=v[1].strip() if len(v)>1 else ""
                        if cod not in TRIBUTO:
                            if form=="1663": cod="PLAME"
                            else: continue
                        imp=num(mstr)
                        if imp<=0: continue
                        fp=None
...
def analizar(extracto, reporte, digito, igv_justo, hoy):
    ...
    for _,fila in extracto.iterrows():
        ...
        # PLAME (1663): no cruzar por código, el reporte tiene el desglose
        if cod=="PLAME": continue
        ...
        if cod=="1011":
            fij=rec_rep.get("igv_justo")  # fecha del reporte tiene PRIORIDAD
            if fij:
                fv=fij; tv="IGV Justo (reporte)"
            elif igv_justo:
                fv=fvenc_ij(anio,mes,digito); tv="IGV Justo (calculado)"
            else:
        ...
        tarde=dt>0
        tim_v=ctim(imp,dt,fv) if tarde else 0.0
        mul_v=0.0
        multa_alerta=afecta_multa(cod, dt)
        ...
        base={"codigo":cod,"nombre":TRIBUTO[cod],
              ...
              "tiene_multa":multa_alerta,"n_orden":nord,"ya_reg":ya,
    ...
    for r in reporte:
        ...
        if cod=="8021":
            ...
            vn.append({
                ...
                "dias_tarde":dv,"tarde":True,"tim":ctim(r["declarado"],dv,fv),"multa":0,
        ...
        # ── CLAVE: Si hay pago PLAME (1663) del mismo período en el extracto
        # → el tributo probablemente fue pagado pero no registrado en el reporte
        # → mostrar como "falta actualizar" en vez de "vencido"
        hay_plame_mismo_periodo = (
            not extracto.empty and
            any(str(f["COD"])=="PLAME" and str(f["PERIODO"])==f"{anio}{mv:02d}"
                for _,f in extracto.iterrows())
        )
        if hay_plame_mismo_periodo and cod not in ("1011","8021","AFP","SIS"):
            # Hay pago PLAME del período → marcar como "falta actualizar"
            # No sabemos la fecha exacta del pago para este tributo específico
            # Tomamos la fecha del pago PLAME más reciente del período
            pagos_plame_per = [
                f for _,f in extracto.iterrows()
                if str(f["COD"])=="PLAME" and str(f["PERIODO"])==f"{anio}{mv:02d}"
            ]
            fecha_pago_plame = max(
                (f["FECHA"] for f in pagos_plame_per if f["FECHA"] is not None),
                default=None
            )
            pest = {
                "5210":"ESSALUD","5310":"ONP","3052":"RENTA_5TA",
                "3042":"RENTA_4TA","3038":"ITAN","AFP":"AFP",
            }.get(cod, f"RENTA {anio}")
            mn_s = mn if fecha_pago_plame is None else mn
            fp_s = fecha_pago_plame.strftime("%d/%m/%Y") if fecha_pago_plame else "ver extracto"
            n_ord = next(
                (str(f["N_ORDEN"]) for _,f in extracto.iterrows()
                 if str(f["COD"])=="PLAME" and str(f["PERIODO"])==f"{anio}{mv:02d}"),
                ""
            )
            instr = f'Pestaña "{pest}" → {mn} → SE PAGÓ = S/ {saldo:,.2f} · {fp_s}'
            act.append({
                "codigo":cod,"nombre":TRIBUTO.get(cod,""),
                "periodo":f"{mn}-{anio}","mes":mn,"mes_num":mv,"anio":anio,
                "importe":saldo,"fecha_pago":fecha_pago_plame,
                "fecha_venc":fvenc(anio,mv,digito),"tipo_venc":"Normal",
                "dias_tarde":0,"tarde":False,"tim":0,"multa":0,
                "tiene_multa":False,"n_orden":n_ord,"ya_reg":False,
                "instruccion":instr,"es_manual":False})
            continue
        # ── IGV: buscar pago directo en extracto (viene con COD="1011", no PLAME) ──
        if cod=="1011" and not extracto.empty:
            pago_igv = next(
                (f for _,f in extracto.iterrows()
                 if str(f["COD"])=="1011"
                 and str(f["PERIODO"])==f"{anio}{mv:02d}"
                 and abs(float(f["IMPORTE"])-saldo)<1.0),
                None
            )
            if pago_igv is not None:
                fp_igv = pago_igv["FECHA"] if isinstance(pago_igv["FECHA"], date) else None
                fp_s = fp_igv.strftime("%d/%m/%Y") if fp_igv else "ver extracto"
                nord_igv = str(pago_igv["N_ORDEN"])
                act.append({
                    "codigo":cod,"nombre":TRIBUTO.get(cod,""),
                    "periodo":f"{mn}-{anio}","mes":mn,"mes_num":mv,"anio":anio,
                    "importe":saldo,"fecha_pago":fp_igv,
                    "fecha_venc":fvenc(anio,mv,digito),"tipo_venc":"Normal",
                    "dias_tarde":0,"tarde":False,"tim":0,"multa":0,
                    "tiene_multa":False,"n_orden":nord_igv,"ya_reg":False,
                    "instruccion":f'Pestaña "IGV {anio}" → {mn} → SE PAGÓ = S/ {saldo:,.2f} · {fp_s}',
                    "es_manual":False})
                continue
        ...
        if cod=="1011":
            ...
            if fij:
                ...
                fv=fvenc_ij(anio,mv,digito); tv="IGV Justo (calculado)"
            else:
...
# Mensajes aprobados por Contadeus para el informe al cliente
MENSAJE_CLIENTE = {
...
def generar_informe_cliente(empresa, res, hoy):
    ...
    lines=[]
    # Encabezado: nombre empresa si disponible, sino RUC
    nombre_display = empresa if empresa and not empresa.isdigit() else f"RUC {empresa}"
    lines.append(f"📊 *Reporte de Impuestos*")
    lines.append(f"*{nombre_display}*")
    lines.append(f"_{hoy.strftime('%d/%m/%Y')}_")
    lines.append("")
    ...
    if not todos:
        ...
        lines.append("")
        lines.append("*Contadeus International SAC*")
        return "\n".join(lines)
    ...
    for i, r in enumerate(ordenados, 1):
        ...
        # Detalle para el cliente — mensaje aprobado por Contadeus
        msg = MENSAJE_CLIENTE.get(r["codigo"], "")
        if r["dias_tarde"] > 0 and not r.get("es_manual"):
            detalle = f"_({r['dias_tarde']} días de retraso. {msg})_" if msg else f"_({r['dias_tarde']} días de retraso, interés incluido)_"
        elif msg:
            detalle = f"_({msg})_"
        else:
            detalle = ""
    ...
    lines.append("")
    lines.append("Hemos identificado estas obligaciones como prioritarias. Recomendamos regularizarlas según el orden indicado para mantener su empresa al día y evitar contingencias futuras.")
    lines.append("")
    lines.append("Si desea efectuar algún pago, indíquenos cuál y con gusto le enviaremos el NPS o guía de pago.")
    lines.append("")
...
def gen_excel(empresa,ruc,res,extracto,noms,fuente):
    ...
    with pd.ExcelWriter(buf,engine="openpyxl") as w:
        ...
        def rows(lst,extras):
            ...
            for r in lst:
                ...
                for k in extras:
                    ...
                    elif k=="Multa": d[k]="Afecto a multa - revisar gradualidad" if r.get("tiene_multa") else "No aplica"
        ...
        if vm:  pd.DataFrame(rows(vm, ["Vencimiento","Días","TIM S/","Multa","Total S/"])).to_excel(w,sheet_name="🔴 VENCIDOS+MULTA",index=False)
        if vn:  pd.DataFrame(rows(vn, ["Vencimiento","Días","TIM S/","Total S/"])).to_excel(w,sheet_name="🔴 VENCIDOS SIN PAGAR",index=False)
        if act: pd.DataFrame(rows(act,["Instrucción","Días","TIM S/","Multa"])).to_excel(w,sheet_name="🟡 ACTUALIZAR REPORTE",index=False)
        if atr: pd.DataFrame(rows(atr,["Vencimiento","Días","TIM S/","Multa","Tipo"])).to_excel(w,sheet_name="🟠 PAGADOS CON ATRASO",index=False)
...
if st.session_state.listo and st.session_state.res:
    ...
    if vm:
        st.markdown('<div class="stitle">🚨 URGENTE — Retenciones vencidas con riesgo de multa adicional</div>',
                    unsafe_allow_html=True)
        st.markdown('<div class="card cr">ONP / Renta 5ta / Renta 4ta / Renta 2da vencidos. '
                    'SUNAT puede aplicar <strong>multa adicional del 100% del tributo omitido</strong>. '
                    'Pagar hoy.</div>',unsafe_allow_html=True)
        for anio in anios:
    ...
    if atr:
        st.markdown('<div class="sep"></div>',unsafe_allow_html=True)
        with st.expander(f"🟠 {len(atr)} pagados con atraso — TIM y multas aplicadas"):
            for anio in anios:
                items=[r for r in atr if r["anio"]==anio]
                if not items: continue
                st.markdown(f'<div class="yr">📅 {anio}</div>',unsafe_allow_html=True)
                for r in items:
                    ml='⚠️ Afecto a multa' if r.get("tiene_multa") else "Solo TIM"
...
# ── FORMULARIO ──────────────────────────────────────────────────────────────
    ...
    with col_h:
        ...
        st.markdown("""<div class="card cr" style="margin-top:10px;">
            ...
            <em>El sistema solo alerta. Contadeus revisa gradualidad y monto manualmente.</em>
        </div>""",unsafe_allow_html=True)
