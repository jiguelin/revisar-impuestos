...
TRIBUTO = {
    "1011":"IGV","1041":"IGV Servicios No Domiciliados","3031":"Impuesto a la Renta","3111":"Impuesto a la Renta",
...
SMAP=[
    ...
    (r"IGV.*NO DOMIC|IGV.*SERV.*NO|NO DOMIC.*IGV",                  "1041","B"),
...
def analizar(extracto, reporte, digito, igv_justo, hoy):
    ...
    for r in reporte:
        ...
        # Para IGV (1011): solo si hay un pago PLAME con monto exacto igual al IGV
        # (el IGV puede venir en 1663 excepcionalmente, como en ESSENTTA)
        # Para otros tributos: cualquier PLAME del período es suficiente
        if cod == "1011":
            montos_plame = [
                f["IMPORTE"] for _,f in extracto.iterrows()
                if str(f["COD"])=="PLAME" and str(f["PERIODO"])==f"{anio}{mv:02d}"
            ]
            hay_plame_igv = any(abs(m - saldo) <= 1.0 for m in montos_plame)
            hay_plame_mismo_periodo = hay_plame_igv

        if hay_plame_mismo_periodo and cod not in ("8021",):
...
CONCEPTO_CLIENTE = {
    ...
    "1011": "IGV",
    "1041": "IGV Servicios No Domiciliados",
...
MENSAJE_CLIENTE = {
    ...
    "1041": "Incluye intereses acumulados a la fecha.",
