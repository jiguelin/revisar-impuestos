...
def limpiar_num(s):
    """2,681 → 2681.0  |  (321) → 321.0  |  0.00 → 0.0"""
...
def detectar_seccion_titulo(texto):
    ...
    if not anio_m: return None,None,None
...
def parsear_fila_reporte(vals, tipo, seccion_cod):
    ...
    for i in range(len(v)-1,-1,-1):
        ...
        if c.startswith('('): continue  # negativo = pagado
        n=limpiar_num(c)
        if n>=0:
            pendiente=n; break
    ...
    importe_decl=0.0
    for c in v[2:5]:
        if c and not c.startswith('(') and c not in ['FRACCIONADO','-']:
            n=limpiar_num(c)
            ...
            for page in pdf.pages:
                for table in page.extract_tables():
                    ...
                    for row in table:
                        ...
                        # ── Parsear fila de mes ──────────────────────────────
                        # Quitar primera celda vacía si existe
    ...
    for _,fila in extracto.iterrows():
        ...
        for s in saldos_reporte:
            if s['codigo']==codigo and s['mes_num']==mes and s['anio']==anio:
                # Está en la sección → hay registro
                # Si pendiente=0 → completamente pagado → registrado
                # Si pendiente>0 pero importe_decl-pendiente≈importe → pago parcial registrado
                ya_reg=True; break
        # Si el tributo/período no aparece en el reporte → no registrado
        # (FRACCIONAMIENTOS nunca están en el reporte como sección → nunca ya_reg)
