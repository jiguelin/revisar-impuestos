...
<style>
  ...
  /* Informe cliente */
  .inf-box{background:#fff;border:2px solid #1B2A8C;border-radius:12px;padding:20px 22px;margin-top:8px;}
  .inf-title{font-weight:800;color:#1B2A8C;font-size:1rem;margin-bottom:12px;}
  .inf-row{padding:4px 0;font-size:.88rem;border-bottom:1px solid #F3F4F6;}
...
TIM_D = 0.0004   # 0.04% diario — actualizar si SUNAT cambia
CON_MULTA = {"3052","3042","3022","5310"}  # Retenciones → multa 5% UIT
...
TRIBUTO = {
    ...
    "AFP":"AFP","SIS":"Seguro Integral de Salud (SIS)",
}
...
# ── CRONOGRAMA SUNAT OFICIAL ──────────────────────────────────────────────────
# Índices 0-9 = dígito RUC, índice 10 = buenos contribuyentes
# Fuente: El Peruano, resoluciones oficiales
CRON = {
    # 2022: RS 189-2021/SUNAT — fuente: sunat.gob.pe/orientacion/cronogramas/2022
    2022:{
        1: [15,16,17,17,18,18,21,21,22,22,23],  # vence FEB-22
        2: [15,16,17,17,18,18,21,21,22,22,23],  # vence MAR-22
        3: [19,20,21,21,22,22,25,25,26,26,27],  # vence ABR-22
        4: [17,18,19,19,20,20,23,23,24,24,25],  # vence MAY-22
        5: [15,16,17,17,20,20,21,21,22,22,23],  # vence JUN-22
        6: [15,18,19,19,20,20,21,21,22,22,25],  # vence JUL-22
        7: [16,17,18,18,19,19,22,22,23,23,24],  # vence AGO-22
        8: [15,16,19,19,20,20,21,21,22,22,23],  # vence SET-22
        9: [17,18,19,19,20,20,21,21,24,24,25],  # vence OCT-22
        10:[15,16,17,17,18,18,21,21,22,22,23],  # vence NOV-22
        11:[15,16,19,19,20,20,21,21,22,22,23],  # vence DIC-22
        12:[17,18,19,19,20,20,23,23,24,24,25],  # vence ENE-23
    },
    # 2023: RS 281-2022/SUNAT — fuente: sunat.gob.pe/orientacion/cronogramas/2023
    2023:{
        1: [15,16,17,17,20,20,21,21,22,22,23],  # vence FEB-23
        2: [15,16,17,17,20,20,21,21,22,22,23],  # vence MAR-23
        3: [19,20,21,21,24,24,25,25,26,26,27],  # vence ABR-23
        4: [16,17,18,18,19,19,22,22,23,23,24],  # vence MAY-23
        5: [15,16,19,19,20,20,21,21,22,22,23],  # vence JUN-23
        6: [17,18,19,19,20,20,21,21,24,24,25],  # vence JUL-23
        7: [15,16,17,17,18,18,21,21,22,22,23],  # vence AGO-23
        8: [15,18,19,19,20,20,21,21,22,22,25],  # vence SET-23
        9: [16,17,18,18,19,19,20,20,23,23,24],  # vence OCT-23
        10:[16,17,20,20,21,21,22,22,23,23,24],  # vence NOV-23
        11:[18,19,20,20,21,21,22,22,26,26,27],  # vence DIC-23
        12:[16,17,18,18,19,19,22,22,23,23,24],  # vence ENE-24
    },
        2024:{
    ...
    # 2025: CRONOGRAMA OFICIAL CORREGIDO (El Peruano)
    2025:{
        1:[17,18,19,19,20,20,21,21,24,24,25],  # vence FEB-25
        2:[17,18,19,19,20,20,21,21,24,24,25],  # vence MAR-25
        3:[15,16,21,21,22,22,23,23,24,24,25],  # vence ABR-25
        4:[16,19,20,20,21,21,22,22,23,23,26],  # vence MAY-25
        5:[16,17,18,18,19,19,20,20,23,23,24],  # vence JUN-25
        6:[15,16,17,17,18,18,21,21,22,22,24],  # vence JUL-25
        7:[18,19,20,20,21,21,22,22,25,25,26],  # vence AGO-25
        8:[15,16,17,17,18,18,19,19,22,22,23],  # vence SEP-25
        9:[16,17,20,20,21,21,22,22,23,23,24],  # vence OCT-25
        10:[17,18,19,19,20,20,21,21,24,24,25], # vence NOV-25
        11:[17,18,19,19,22,22,23,23,24,24,26], # vence DIC-25
        12:[16,19,20,20,21,21,22,22,23,23,26], # vence ENE-26
    },
    2026:{
...
def fvenc_ij(anio, mes, dig):
    """IGV Justo = vencimiento del período siguiente."""
    m2,a2 = mes+1,anio
    if m2>12: m2,a2=1,anio+1
    return fvenc(a2,m2,dig)

def ctim(imp, dias): return round(imp*TIM_D*max(dias,0), 2)
def cmulta(anio):    return UIT.get(anio,5500)*0.05

def num(s):
    """Convierte cualquier formato numérico a float. Soporta (321)/-321/2,681/2.681"""
...
# ══════════════════════════════════════════════════════════════════════════════
# GOOGLE SHEET — lectura directa via CSV export
# ══════════════════════════════════════════════════════════════════════════════
def sid(url):
...
def leer_sheet(sheet_id):
    """Lee el Sheet como CSV. Intenta pestaña 'REPORTE DE IMPUESTOS' primero."""
...
SMAP=[
    ...
    # AFP y SIS: manual, pero los mostramos como pendientes
    (r"\bAFP\b",                                                  "AFP","C"),
    (r"\bSIS\b",                                                  "SIS","C"),
]
...
def pend_fila(vals):
    """Pendiente = último valor positivo (no pago) de derecha a izquierda."""
...
def parsear_reporte(df):
    """
    Parsea el DataFrame del reporte de impuestos.
    Devuelve lista de registros por sección+período.
    Incluye AFP y SIS como pendientes manuales.
    """
    recs=[]; vistos=set()
    sc=sa=st_=None
    ...
    for _,row in df.iterrows():
        ...
        recs.append({"tipo":st_,"codigo":sc,"nombre":TRIBUTO.get(sc,sc),
                     ...
                     "vcto_sheet":None,"igv_justo":igv_j,"es_manual":es_manual})
    return recs
...
def combinar_sunat(archs):
    ...
    dup=antes-len(c)
    if dup>0: st.info(f"ℹ️ {dup} duplicados eliminados.")
    return c.reset_index(drop=True),noms

# ══════════════════════════════════════════════════════════════════════════════
# ANÁLISIS CENTRAL
# Produce 5 categorías en orden de prioridad:
#   1. VENCIDOS CON MULTA — retenciones vencidas (ONP/R5ta/R4ta/R2da)
#   2. VENCIDOS SIN MULTA — IGV/Renta/EsSalud vencidos
#   3. FALTA ACTUALIZAR   — pagado en SUNAT pero no en reporte
#   4. CON ATRASO         — pagado tarde (TIM generado)
#   5. AL DÍA             — correcto
# ══════════════════════════════════════════════════════════════════════════════
def analizar(extracto, reporte, digito, igv_justo, hoy):
    venc_multa=[]   # 1. vencidos con multa (retenciones)
    venc_normal=[]  # 2. vencidos sin multa
    actualizar=[]   # 3. falta actualizar
    con_atraso=[]   # 4. pagado tarde
    al_dia=[]       # 5. correcto
    vistos=set()

    # ── Paso 1: Pagos del extracto SUNAT ─────────────────────────────────────
    for _,fila in extracto.iterrows():
        ...
        if cod=="1011" and igv_justo:
            ...
            fv=next((r["vcto_sheet"] for r in reporte
                    ...
                    and abs(r["declarado"]-imp)<1.0),None); tv="Fracc"
        else:
            fv=fvenc(anio,mes,digito); tv="Normal"

        # ¿Registrado en reporte?
        if cod=="8021":
            ya=any(r["codigo"]=="8021" and r["anio"]==anio
                  and r.get("pagada") and abs(r.get("pagado_reg",0)-imp)<1.0
                  for r in reporte)
        else:
            ya=any(r["codigo"]==cod and r["mes_num"]==mes and r["anio"]==anio
                  for r in reporte)
        ...
        if not ya:
            actualizar.append(base)
        elif tarde and (tim_v>0 or mul_v>0):
            con_atraso.append(base)
        else:
            al_dia.append(base)

    # ── Paso 2: Saldos vencidos del reporte ──────────────────────────────────
    for r in reporte:
        ...
        if es_manual:
            # No tiene vencimiento SUNAT, mostrar como pendiente manual
            venc_normal.append({
                "codigo":cod,"nombre":TRIBUTO.get(cod,cod),
                "periodo":f"{mn}-{anio}","mes":mn,"mes_num":mv,"anio":anio,
                "importe":saldo,"fecha_pago":None,
                "fecha_venc":None,"tipo_venc":"Manual",
                "dias_tarde":0,"tarde":False,"tim":0,"multa":0,
                "tiene_multa":False,"n_orden":"","ya_reg":True,
                "instruccion":f"AFP/SIS — pago manual pendiente S/ {saldo:,.2f}",
                "es_manual":True})
            continue
        ...
        if fv>=hoy: continue  # aún no vence
        ...
        if cod in CON_MULTA:
            venc_multa.append(item)
        else:
            venc_normal.append(item)

    # Ordenar por urgencia
    venc_multa  = sorted(venc_multa,  key=lambda x:x["dias_tarde"],reverse=True)
    venc_normal = sorted(venc_normal, key=lambda x:x["dias_tarde"],reverse=True)
    con_atraso  = sorted(con_atraso,  key=lambda x:x["dias_tarde"],reverse=True)

    return {"venc_multa":venc_multa,"venc_normal":venc_normal,
            "actualizar":actualizar,"con_atraso":con_atraso,"al_dia":al_dia}

# ══════════════════════════════════════════════════════════════════════════════
# INFORME PARA EL CLIENTE (texto para copiar/imagen WhatsApp)
# Prioridad: multas > vencidos > pendientes de pago futuros
# ══════════════════════════════════════════════════════════════════════════════
def generar_informe_cliente(empresa, res, hoy):
    vm=res["venc_multa"]; vn=res["venc_normal"]; act=res["actualizar"]
    lines=[]
    lines.append(f"📊 *ESTADO TRIBUTARIO — {empresa.upper()}*")
    lines.append(f"_Fecha: {hoy.strftime('%d/%m/%Y')}_")
    lines.append("")

    if vm:
        lines.append("🔴 *URGENTE — Con riesgo de multa adicional:*")
        for r in vm:
            mul_txt=f" + Multa S/{r['multa']:,.0f}" if r['multa'] else ""
            lines.append(f"• {r['nombre']} {r['periodo']}: *S/ {r['importe']:,.2f}*{mul_txt} + TIM S/{r['tim']:,.2f} ({r['dias_tarde']}d vencido)")
        lines.append("")

    if vn:
        lines.append("🔴 *Impuestos vencidos sin pagar:*")
        for r in vn:
            if r.get("es_manual"):
                lines.append(f"• {r['nombre']} {r['periodo']}: *S/ {r['importe']:,.2f}* (pago manual pendiente)")
            elif r["fecha_venc"]:
                lines.append(f"• {r['nombre']} {r['periodo']}: *S/ {r['importe']:,.2f}* + TIM S/{r['tim']:,.2f} (venció {r['fecha_venc'].strftime('%d/%m/%Y')})")
            else:
                lines.append(f"• {r['nombre']} {r['periodo']}: *S/ {r['importe']:,.2f}*")
        lines.append("")

    # Pendientes por vencer (del reporte, aún no vencidos)
    if act:
        lines.append("🟡 *Por regularizar en el reporte:*")
        for r in act:
            lines.append(f"• {r['nombre']} {r['periodo']}: S/ {r['importe']:,.2f}")
        lines.append("")

    total_urgente = sum(r["importe"]+r["tim"]+r["multa"] for r in vm+vn)
    if total_urgente > 0:
        lines.append(f"💰 *Total a pagar urgente: S/ {total_urgente:,.2f}*")
        lines.append("")

    if not vm and not vn and not act:
        lines.append("✅ *Todo al día. No hay pagos pendientes.*")

    lines.append("_Contadeus International SAC_")
    return "\n".join(lines)
...
def gen_excel(empresa,ruc,res,extracto,noms,fuente):
    buf=BytesIO()
    vm=res["venc_multa"]; vn=res["venc_normal"]
    act=res["actualizar"]; mul=res["con_atraso"]; ok=res["al_dia"]
    todos_venc=vm+vn
    t_urgente=sum(r["importe"]+r["tim"]+r["multa"] for r in todos_venc)

    with pd.ExcelWriter(buf,engine="openpyxl") as w:
        hs=datetime.now().strftime("%d/%m/%Y %H:%M")
        anios=sorted({r["anio"] for r in vm+vn+act+mul+ok})
        pd.DataFrame({"Campo":[
            "Empresa","RUC","Fecha análisis","PDF SUNAT","Fuente reporte",
            "Años analizados","Pagos en extracto",
            "Vencidos con riesgo multa","Vencidos sin pagar",
            "Falta actualizar reporte","Pagados con atraso","Al día",
            "Total urgente S/",
        ],"Valor":[
            empresa,ruc,hs,", ".join(noms),fuente,
            ", ".join(str(a) for a in anios),len(extracto),
            len(vm),len(vn),len(act),len(mul),len(ok),f"{t_urgente:,.2f}",
        ...
        def rows(lst,extras):
            ...
            for r in lst:
                ...
                for k in extras:
                    ...
                    elif k=="Días vencido": d[k]=r["dias_tarde"]
                    elif k=="TIM S/": d[k]=r["tim"]
                    elif k=="Multa S/": d[k]=r["multa"] if r["multa"] else "No aplica"
                    elif k=="Total regularizar S/": d[k]=r["importe"]+r["tim"]+r["multa"]
                    elif k=="Instrucción": d[k]=r.get("instruccion","")
                    elif k=="Tipo venc.": d[k]=r["tipo_venc"]
                    elif k=="Días tarde": d[k]=r["dias_tarde"]
                out.append(d)
            return out

        if vm:  pd.DataFrame(rows(vm, ["Vencimiento","Días vencido","TIM S/","Multa S/","Total regularizar S/"])).to_excel(w,sheet_name="🔴 VENCIDOS+MULTA",index=False)
        if vn:  pd.DataFrame(rows(vn, ["Vencimiento","Días vencido","TIM S/","Total regularizar S/"])).to_excel(w,sheet_name="🔴 VENCIDOS SIN PAGAR",index=False)
        if act: pd.DataFrame(rows(act,["Instrucción","Días tarde","TIM S/","Multa S/"])).to_excel(w,sheet_name="🟡 ACTUALIZAR REPORTE",index=False)
        if mul: pd.DataFrame(rows(mul,["Vencimiento","Días tarde","TIM S/","Multa S/","Tipo venc."])).to_excel(w,sheet_name="🟠 PAGADOS CON ATRASO",index=False)
...
<div class="hdr">
  ...
  <p>Cruza extracto SUNAT con reporte · Detecta qué actualizar, qué pagar y qué multas hay · Genera informe para el cliente</p>
</div>
""", unsafe_allow_html=True)

SK=["listo","resultado","empresa","ruc","extracto_df","noms","fuente"]
for k in SK:
...
if st.session_state.listo and st.session_state.resultado:
    ...
    anios=sorted({r["anio"] for r in vm+vn+act+mul+ok},reverse=True)

    st.markdown(f'<div class="emp">🏢 {empresa}{f" · RUC {ruc}" if ruc else ""} · Años: {" · ".join(str(a) for a in anios)}</div>',
                unsafe_allow_html=True)

    # KPIs
    for col,(n,lbl,color) in zip(st.columns(5),[
        (len(vm),"Vencidos + multa","red"),(len(vn),"Vencidos sin pagar","red"),
        (len(act),"Falta actualizar","amb"),(len(ok),"Al día","grn"),
        (f"S/ {t_urgente:,.0f}","Total urgente","red")]):
        with col:
            st.markdown(f'<div class="kpi"><div class="n {color}">{n}</div><div class="l">{lbl}</div></div>',
                        unsafe_allow_html=True)

    st.markdown('<div class="sep"></div>',unsafe_allow_html=True)

    # ── 1. VENCIDOS CON MULTA (máxima urgencia) ──────────────────────────────
    if vm:
        st.markdown('<div class="stitle">🔴 URGENTE — Impuestos vencidos con riesgo de multa adicional</div>',
                    unsafe_allow_html=True)
        st.markdown('<div class="card cr">ONP / Renta 5ta / Renta 4ta / Renta 2da vencidos. Si SUNAT detecta el incumplimiento antes del pago puede aplicar <strong>multa adicional del 100% del tributo omitido</strong>. Pagar cuanto antes.</div>',
                    unsafe_allow_html=True)
        for anio in anios:
            ...
            for r in items:
                ...
                st.markdown(f"""<div class="card cr">
                    ...
                    <span class="badge br">Multa S/ {r['multa']:,.0f}</span>
                    <span class="badge ba">TIM S/ {r['tim']:,.2f}</span>
                    <span class="badge br">Total regularizar: S/ {costo:,.2f}</span>
            ...
            for r in items:
                if r.get("es_manual"):
                    st.markdown(f"""<div class="card cp">
                        <strong>📋 {r['nombre']} — {r['periodo']}</strong><br>
                        💰 S/ {r['importe']:,.2f} &nbsp;·&nbsp;
                        <span class="badge bp">Pago manual pendiente</span><br>
                        <span style="color:#6B21A8">▶ Coordinar pago manual con cliente</span>
                    </div>""",unsafe_allow_html=True)
                else:
                    costo=r["importe"]+r["tim"]+r["multa"]
                    vs=r["fecha_venc"].strftime("%d/%m/%Y") if r.get("fecha_venc") else "N/D"
    ...
    if act:
        st.markdown('<div class="sep"></div>',unsafe_allow_html=True)
        st.markdown('<div class="stitle">🟡 Falta actualizar el reporte de impuestos</div>',
                    unsafe_allow_html=True)
        st.markdown('<div class="card ca">El cliente pagó estos impuestos (aparecen en el extracto SUNAT) pero <strong>no están registrados en el reporte</strong>. Actualizar antes de enviar informe al cliente.</div>',
                    unsafe_allow_html=True)
        for anio in anios:
            ...
            for r in items:
                atr=""
                if r["tarde"]:
                    ml=f'<span class="badge br">Multa S/ {r["multa"]:,.0f}</span>' if r["multa"] else ""
                    atr=(f'<br>{ml}<span class="badge ba">TIM S/ {r["tim"]:,.2f}</span>'
                        f' — pagado {r["dias_tarde"]} días tarde')
    ...
    # ── 4. CON ATRASO (registrado pero tarde) ────────────────────────────────
    if mul:
        st.markdown('<div class="sep"></div>',unsafe_allow_html=True)
        with st.expander(f"🟠 {len(mul)} pagados con atraso — TIM y multas aplicables"):
            for anio in anios:
    ...
    # ── 5. AL DÍA ────────────────────────────────────────────────────────────
    if ok:
        with st.expander(f"✅ {len(ok)} pagos al día y correctamente registrados"):
            for r in ok:
                fp_s=r["fecha_pago"].strftime("%d/%m/%Y") if r.get("fecha_pago") else ""
                st.markdown(f'<div class="card cg">✓ {r["nombre"]} — {r["periodo"]} — S/ {r["importe"]:,.2f} — {fp_s}</div>',
                            unsafe_allow_html=True)

    if not vm and not vn and not act and not mul:
        st.markdown('<div class="card cg">✅ <strong>Todo al día.</strong> No hay pendientes ni impuestos por actualizar.</div>',
                    unsafe_allow_html=True)

    # ── INFORME PARA EL CLIENTE ───────────────────────────────────────────────
    st.markdown('<div class="sep"></div>',unsafe_allow_html=True)
    st.markdown('<div class="stitle">📱 Informe para el cliente — copiar y pegar en WhatsApp</div>',
                unsafe_allow_html=True)

    informe=generar_informe_cliente(empresa,res,HOY)
    st.text_area("",value=informe,height=300,
                 help="Copia este texto y pégalo en WhatsApp al cliente.",
                 label_visibility="collapsed")
    st.caption("💡 Selecciona todo el texto → Ctrl+C (o Cmd+C en Mac) → Pegar en WhatsApp")
...
# ── FORMULARIO ──────────────────────────────────────────────────────────────
    ...
    with col_f:
        ...
        if ruc_inp and len(ruc_inp.strip())==11 and ruc_inp.strip().isdigit():
            ...
            st.markdown(f'<div class="card cb" style="font-size:.82rem;padding:7px 12px;">✓ Dígito RUC: <strong>{digito_calc}</strong> — cronograma SUNAT 2024/2025/2026 calculado</div>',
                        unsafe_allow_html=True)
        ...
        st.markdown("")
        st.markdown("### 📄 PDF 1 — Extracto SUNAT (Reporte de Declaraciones y Pagos)")
        st.caption("SUNAT SOL → Mis declaraciones y pagos → Reporte electrónico → PDF. Puedes subir varios años juntos.")
        arch_s=st.file_uploader("",type=["pdf"],accept_multiple_files=True,
                                label_visibility="collapsed",key="us")
        if arch_s:
            st.markdown(f'<div class="card cg" style="font-size:.82rem;padding:7px 12px;">📄 {len(arch_s)} PDF(s): {" · ".join(a.name for a in arch_s)}</div>',
                        unsafe_allow_html=True)
        ...
        with tab_s:
            ...
            if su: st.caption("El Sheet debe estar compartido como 'Cualquier persona con el link puede ver'.")
        with tab_p:
            ar=st.file_uploader("",type=["pdf"],accept_multiple_files=False,
                               label_visibility="collapsed",key="ur")
            if ar:
                st.markdown(f'<div class="card cg" style="font-size:.82rem;padding:7px 12px;">📊 {ar.name}</div>',
                            unsafe_allow_html=True)
        ...
        if st.button("🔍  Analizar",type="primary",use_container_width=True):
            ...
            if not err and digito_calc is not None:
                ...
                if su.strip():
                    ...
                    if rdf is None:
                        st.error("No se pudo leer el Sheet. Verifica que esté compartido como 'Cualquier persona con el link puede ver'.")
                        st.stop()
                elif ar:
                ...
                with st.spinner("Analizando 2022/2023/2024/2025/2026..."):
                ...
                if sum(len(v) for v in resultado.values())==0:
                    ...
                    st.session_state.update({
                        "listo":True,"resultado":resultado,
                        "empresa":ef,"ruc":ruc_inp.strip(),
                        "extracto_df":edf,"noms":noms,"fuente":fuente})
                    st.rerun()

    with col_h:
        st.markdown("""<div class="card cb" style="margin-top:8px;">
            <strong>📋 Qué hace esta app</strong><br><br>
            Cruza el extracto SUNAT con el reporte del cliente:<br><br>
            🔴 <strong>Vencidos + multa</strong> — ONP/R5ta/R4ta/R2da vencidos. Multa crece con el tiempo.<br><br>
            🔴 <strong>Vencidos</strong> — IGV/Renta/EsSalud/Fracc sin pagar. TIM diario.<br><br>
            🟡 <strong>Actualizar reporte</strong> — el cliente pagó pero no está en el reporte.<br><br>
            🟠 <strong>Con atraso</strong> — pagado fuera de fecha. TIM generado.<br><br>
            ✅ <strong>Al día</strong> — correcto.<br><br>
            📱 <strong>Informe WhatsApp</strong> — texto listo para copiar y enviar al cliente.
        </div>""",unsafe_allow_html=True)

        st.markdown("""<div class="card cr" style="margin-top:10px;">
            <strong>⚠️ Multa 5% UIT por pago tardío</strong><br>
            ONP · Renta 5ta · Renta 4ta · Renta 2da<br>
            <em>UIT 2024=S/5,150 · 2025=S/5,350 · 2026=S/5,500</em>
        </div>""",unsafe_allow_html=True)
