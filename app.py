...
def analizar(extracto, reporte, digito, igv_justo, hoy):
    ...
    plame_usados = set()  # índices de filas del extracto ya asignadas a un tributo
    for r in reporte:
        ...
        # ── CLAVE: Si hay pago PLAME (1663) o IGV directo del mismo período
        # Y del MISMO MONTO en el extracto → el tributo fue pagado pero no
        # registrado en el reporte → mostrar como "falta actualizar".
        #
        # Un Formulario 1663 (PLAME) puede agrupar varios tributos (EsSalud,
        # ONP, retenciones de renta) en un solo pago, pero el reporte SUNAT
        # NO desglosa cuánto corresponde a cada uno — solo trae un monto total
        # por N° de orden. Por eso NUNCA se debe dar por pagado un tributo
        # solo porque "algún" PLAME cayó en el mismo período: hay que exigir
        # coincidencia de MONTO exacto (tolerancia S/1) y, una vez usado un
        # pago para un tributo, no se puede reutilizar para otro (se "consume").
        cod_extracto = "1011" if cod=="1011" else "PLAME"
        pago_match = next(
            (f for i,f in extracto.iterrows()
             if str(f["COD"])==cod_extracto
             and str(f["PERIODO"])==f"{anio}{mv:02d}"
             and abs(float(f["IMPORTE"])-saldo)<1.0
             and i not in plame_usados),
            None
        )
        if pago_match is not None and cod not in ("8021","AFP","SIS"):
            plame_usados.add(pago_match.name)
            fp_m = pago_match["FECHA"] if isinstance(pago_match["FECHA"], date) else None
            fp_s = fp_m.strftime("%d/%m/%Y") if fp_m else "ver extracto"
            n_ord = str(pago_match["N_ORDEN"])
            pest = {
                "5210":"ESSALUD","5310":"ONP","3052":"RENTA_5TA",
                "3042":"RENTA_4TA","3038":"ITAN","1011":f"IGV {anio}",
            }.get(cod, f"RENTA {anio}")
            instr = f'Pestaña "{pest}" → {mn} → SE PAGÓ = S/ {saldo:,.2f} · {fp_s}'
            act.append({
                "codigo":cod,"nombre":TRIBUTO.get(cod,""),
                "periodo":f"{mn}-{anio}","mes":mn,"mes_num":mv,"anio":anio,
                "importe":saldo,"fecha_pago":fp_m,
